...
func (r *TodoItemPostgres) GetById(userId, itemId int) (todo.TodoItem, error) {
	...
	query := fmt.Sprintf("SELECT ti.id, ti.title, ti.description, ti.done FROM %s ti INNER JOIN %s li ON li.item_id = ti.id INNER JOIN %s ul ON ul.list_id = li.list_id WHERE li.id = $1 AND ul.user_id = $2",
		todoItemsTable, listsItemsTable, usersListsTable)
...
func (r *TodoItemPostgres) Delete(userId, itemId int) error {
	query := fmt.Sprintf("DELETE FROM %s ti USING %s li, %s ul WHERE ti.id = li.item_id AND li.list_id = ul.list_id AND ul.user_id = $1 AND ti.id = $2",
		listsItemsTable, listsItemsTable, usersListsTable)
	del, err := r.db.Exec(query, userId, itemId)

	if result, _ := del.RowsAffected(); result == 0 {
		err = fmt.Errorf("item does not exist for user")
	}
	return err
}
...
func (r *TodoItemPostgres) Update(userId, itemId int, input todo.UpdateItemInput) error {
	...
	if result, _ := update.RowsAffected(); result == 0 {
		err = errors.New("list does not exist for user")
	}
